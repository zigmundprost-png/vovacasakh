...
const SCENARIOS = [
  ...
  "Хочется, чтобы мастер по-человечески сказал: ремонтировать или нет",
  "Нужен мастер, который заберёт мебель и привезёт обратно — без хлопот",
];
    ...
    <Section
      ...
      intro={
        <>
          ...
          <p className="mt-4">
            В такой момент обычно непонятно три вещи: можно ли это починить,
            сколько будет стоить ремонт и кто возьмёт на себя перевозку, если
            дома работать неудобно. Я помогу разобраться.
          </p>
        </>
      }
    >
